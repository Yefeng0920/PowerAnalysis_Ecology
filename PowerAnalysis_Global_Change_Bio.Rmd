---
title: "The script for the paper 'Low statistical power and overestimated anthropogenic impacts, exacerbated by publication bias, dominate field studies in global change biology'"
author: "Yefeng Yang | Helmut Hillebrand| Malgorzata Lagisz1 | Ian Cleasby | Shinichi Nakagawa"
date: "2021/9/27"
...
# The sampling variance of a folded distribution   
...
# Function to visualize the relationship between effect size, power, Type M / S errors
plot_p_M_S <- function (A, s, alpha = 0.05, df = Inf, n.sims = 5000, gg = TRUE) 
{
  z <- qt(1 - alpha/2, df)
  p.hi <- 1 - pt(z - A/s, df)
  p.lo <- pt(-z - A/s, df)
  ...
  # plot
    ggplot2::ggplot(as.data.frame(estimate), ggplot2::aes(x = seq_along(estimate), 
      y = estimate)) + ggplot2::geom_point(pch = ifelse((estimate > 
      s * z) & (!estimate < -s * z), 15, ifelse((!estimate > 
      s * z) & (estimate < -s * z), 17, ifelse((!estimate > 
      s * z) & (!estimate < -s * z), 16, 16))), col = ifelse((!estimate > 
      s * z) & (estimate < -s * z), "blue", ifelse((estimate > 
      s * z) & (!estimate < -s * z), "red", "grey"))) + 
      ggplot2::geom_hline(yintercept = s * z, size = 1) + 
      ggplot2::geom_hline(yintercept = -s * z, size = 1) + 
      ggplot2::geom_hline(yintercept = A, linetype = "dashed", 
        size = 1) + ggplot2::xlab("n") + ggplot2::labs(y = "Effect size") + ggplot2::theme_bw() + ggplot2::theme(axis.text = element_text(size = 15), axis.title = element_text(size = 15))
}
...
We used a three-step modelling procedure to achieve our aims (that is, quantifying the statistical power, magnitude [Type M error] and sign errors [Type S error] in global change studies). 
...
### 1. SMD and lnRR
This section is to calculate power, Type M / S errors in meta-analysislevel and single experiment level for lnRR* (see section 2.2 for demographics)

#### 1.1 Meta-analysis level
```{r}
#***************************************************************#
...
# calculate 2 types of effect size statistics (i.e.SMD and lnRR)
SMD <- NA
...
#### 1.2 Single experiment level
##### 1.2.1 Calculation
We have two true effect proxies for calculating experiment level power: (i) meta-analytic overall mean (which is same as the meta-analysis level calculation), (2) experiment-specific effect (also termed as effect size specific effect).

Experiment-specific effect is an alternative proxy for 'true' effect. Traditional approach of obtaining a 'true' effect for power calculation is to use the meta-analytic overall mean, in which we assume that all experiments included in a meta-analysis share a common 'true' effect (that is, meta-analytic mean). However, multiple true effects within one meta-analysis might exist – experiment-specific effect. Because the true effects could be conditional on the certain experimental contexts. For example, the nutrient enrichment generally has a large effect on plant biomass, however, lack of light stimuli will largely reduce this effect. This is an indication that within a meta-analysis, there might exist multiple true effects. Eq's 10 (see main text) is used to such experiment-specific effects. Technically, experiment-specific effects are observation (experiment) level estimates conditional on a set of random effects in a mixed effect model (in this case, random effects represent the various experimental contexts).
...
# Use effects without random effects as true effect sizes for each single experiment - which can account for random effects (i.e., experiment-context dependent) of each study
...
##### 1.2.2 Summary of experimental power
```{r}
#*********************************************************************#
#----------- summary of experimental power for each approach ------------#
#*********************************************************************#
...
##### 1.2.3 SE of experimental power
...
#### 1.3 Aggretation across meta-analyses 
...
We used weighted regression to statistically aggregate over the three parameters obtained at the within-meta-analysis level whereas we used mixed effects models to aggregate these parameters at the experiment level. Both procedures involved aggregating the parameters across meta-analyses (i.e., between-meta-analysis modelling; Section 2.5.2). 
...
individual_est_SMD <- data.frame("MA_case_SMD" = MA_case_SMD,
...
#***************************************************************#
#        estiamte overall power for experiment level power      #
#***************************************************************#
#----------------------------------------------------------------#
#           (i) intercept (overall mean) as true effect
#----------------------------------------------------------------#
...
#***************************************************************#
#        estimate overall power for experiment level power      #
#***************************************************************#
#----------------------------------------------------------------#
#           (i) intercept (overall mean) as true effect
#----------------------------------------------------------------#
...
### 2. SMDH 
This section is used to obtain the estimates (power, Type M / S errors) for SMDH, in which we used the same procedures as lnRR and SMD

#### 2.1 Meta-analysis level
```{r}
#***************************************************************#
#             power for 12 meta-analytic cases                  #
#***************************************************************#
...
##### 2.2.2 Summary of experimental power
```{r}
#*********************************************************************#
#----------- summary of experimental power for each approach ------------#
#*********************************************************************#
...
##### 2.2.3 SE of experimental power
```{r}
#*********************************************************************#
#--------- calculate standard error for each type of power -----------#
#*********************************************************************#
...
#### 2.3 Aggretation across meta-analyses 
##### 2.3.1 MA power
```{r}
...
### 3. lnRR*
This section is to obtain estimates (power, Type M / S errors) for lnRR*. The procedures used were same with SMD, lnRR and SMDH

#### 3.1 Meta-analysis level
```{r}
#***************************************************************#
#                  meta-analysis level power                    #
#***************************************************************#

# Importing data # also can use list.files(path = "./36_lnRR_data")
...
# We note that there are some mising values of effect size corresponding variance (i.e. vi) in the original dataset: dat_m17_1, dat_m17_2, dat_m18_1, dat_m19_1, dat_m21_1, dat_m22_1. These datasets can not be used to calculate power. Therefore, we deleted these six datasets. Finally, we have 36-6=30 datasets.
...
##### 3.2.2 Summary of experiment level estimates
```{r}
...
power.F_summary_RR <- data.frame(MA_case=model_est_RR$MA_case,
                   ...
                   Maximum=mapply(summary, sapply(dat_list, function(x) x$power.F_RR))[6,])  # alternatively, sapply(mapply(summary, sapply(dat_list, function(x) x$power_RR)), function(x) x[2])
...
##### 3.2.3 SE of experiment level estimates
```{r}
#*********************************************************************#
#--------- calculate standard error for each type of power -----------#
#*********************************************************************#
...
#### 3.3 Aggretation across meta-analyses 
This section is used to obtain overall estimates of the three parameters across different meta-analyses (which provided us with comparable summaries of the three parameters).
...
##### 3.3.1 MA power
```{r}
# Add k and N to the dataset - ks will be the weights for regression
N_RR <- NA
...
# We need to adjust S by adding a small value - better to just add 0.025
...
### 4. Bias-corrected SMD 
This section is to calculate power, Type M / S errors for bias-corrected SMD

The prerequisite is to obtain the estiamtes of bias-corrected lnRR*. To achieve this, we used an extended version of Egger’s regression approach (multilevel version of PET-PEESE approac). In brief, this approach incorporates uncertainty term as a moderator in a multilevel meta-regression model: the inverse of ‘effective sample size’ or its square root (see more details in section 2.4).

#### 4.1 Estimate bias-adjusted SMD (bias-corrected version SMD)
```{r}
...
#### 4.2 Compile datasets for adjusted SMD
We had 5 meta-analyses showing the correct direction slope. For the left 7 meta-analyses, we used original meta-analysis overall mean
```{r}
#***************************************************************#
#                           Model estimates                     #
#***************************************************************#
...
model_est_adjusted_SMD <- data.frame(MA_case=model_est_PET_PEESE_SMD$MA_case,
                    mu_SMD=c(model_est_PET_PEESE_SMD$mu_SMD[1], # match with the my.data3$ID; also see 1, 3, 4, 11, 12
...
# get models from bias-corrected models (model_est_PET_PEESE_SMD) if in a correct direction of slope. Otherwise it should use original models (model_est_SMD). Using view(my.data3) to get locations of those from bias-corrected models: 1, 3, 4, 11, 12
...
##### 4.4.2 Summary of experimental power
```{r}
#*********************************************************************#
#----------- summary of experimental power for each approach ------------#
#*********************************************************************#
...
##### 4.4.3 SE of experimental power
...
#### 4.5 Aggretation across meta-analyses 
...
### 5. Bias-corrected lnRR 

#### 5.1 Estimate bias-adjusted lnRR (bias-corrected version lnRR)
...
We had 6 meta-analyses showing the correct direction slope. For the left 6 meta-analyses, we used original meta-analysis overall mean
...
##### 5.4.2 Summary of experimental power
...
##### 5.4.3 SE of experimental power
...
#### 5.5 Aggretation across meta-analyses 
...
### 6. Bias-corrected SMDH 

#### 6.1 Estimate bias-adjusted SMDH (bias-corrected version SMDH)
...
#### 6.2 Compile datasets for adjusted SMDH
We  had 5 meta-analyses showing the correct direction slope. For the left 9 meta-analyses, we used original meta-analysis overall mean
...
##### 6.4.2 Summary of experimental power
...
##### 6.4.3 SE of experimental power
...
#### 6.5 Aggretation across meta-analyses 
...
### 7. Bias-corrected lnRR* 
This section is to calculate power, Type M / S errors for bias-corrected lnRR*

The prerequisite is to obtain the estiamtes of bias-corrected lnRR*. To achieve this, we used an extended version of Egger’s regression approach (multilevel version of PET-PEESE approac). In brief, this approach incorporates uncertainty term as a moderator in a multilevel meta-regression model: the inverse of ‘effective sample size’ or its square root (see more details in section 2.4).

#### 7.1 Estimate publication bias adjusted lnRR* (bias-corrected version)
```{r}
...
model_est_PET_PEESE_RR.var.slope26 <- data.frame(MA_case=model_est_PET_PEESE_RR.sei.slope$MA_case[26],
                  ...
                  ll_NHST=0,
                  ul_NHST=0,
                  p_value=1) # For the case 26, because we could not fit PEESE model, so w can not obtain corresponding estimates. We will use estimates from PET, so here we arbitaryly assume estiamtes from PEESE,
...
model_est_PET_PEESE_RR <- model_est_PET_PEESE_RR[order(model_est_PET_PEESE_RR$id),] # reorder according to the orders of meta-analyses
...
png(filename = "./Figures/paired lnRR*.png", width = 5, height = 4, units = "in", type = "windows", res = 400)
paired.plot3 
dev.off()
...
#### 7.2 Compile datasets for adjusted lnRR*
We had 20 meta-analyses showing the correct direction slope. For the left 10 meta-analyses, we used original meta-analysis overall mean instead
...
# using effective N based estimates where possible (using lnRR* dataset's bias-corrected estimates to replace lnRR* dataset's bias-corrected estimates where possible)
...
##### 7.4.2 Summary of experimental power
...
power.F_summary_adjusted_RR <- data.frame(MA_case=model_est_adjusted_RR$MA_case,
                   ...
                   Maximum=mapply(summary, sapply(dat_list_adjusted, function(x) x$power.F_RR))[6,])  
...
##### 7.4.3 SE of experimental power
...
#### 7.5 Aggretation across meta-analyses 
...
### 8. lnCVR and lnVR
This section is to obtain the statistical power, Type M / S errors for response varibility effect sizes - lnCVR and lnVR

#### 8.1 Meta-analysis level
```{r}
#***************************************************************#
#             power for 12 meta-analytic cases                  #
#***************************************************************#
...
##### 8.2.2 Summary of experimental power
...
##### 8.2.3 SE of experimental power
...
#### 8.3 Aggretation across meta-analyses 
...
### The calculations of 'sample size'
This section is used to calculate: (i) replicates of manipulative experiments and non-manipulative observations, and (ii) the number of sample sizes and studies for each type of effect size 
...
### Meta-meta-analysis
This section is to conduct a secondary synthesis of the true effects (which were estimated from the first step) across meta-analyses. That is, conducting a meta-analysis of overall means obtained from the included 30 meta-analyses; also referred to as a second-order meta-analysis or meta-meta-analysis
...
orchard_magnitude_variance <- orchaRd::orchard_plot(MMA_magnitude_variance_results, mod = "Int", xlab = "The effect of stressors on ecosystem response variability", k = FALSE, transfm = "none", angle = 0)+ xlim(-0.19, 1.2) + scale_y_discrete(labels = c("lnCVR", "lnVR")) + labs(title = "(C)", y = "") +
  theme(axis.text.x = element_text(size = 10, colour = "black"),
        ...
        axis.title.x = element_text(size = 10, colour = "black")) +
annotate(geom = "text",
x = 0.8, y = 1.2, size = 3,label = paste0("Estimate = ", round(table_results$estimate[[1]],
3), "\n", "[95% CI = ", round(table_results$lowerCL[[1]], 3),  " to ", round(table_results$upperCL[[1]], 3), "]", "\n", "[95% PI = ",
round(table_results$lowerPR[[1]], 3), " to ",
round(table_results$upperPR[[1]], 3), "]")) +
...
annotate(geom = "text",
x = 0.8, y = 2.2, size = 3,label = paste0("Estimate = ", round(table_results$estimate[[2]],
3), "\n", "[95% CI = ", round(table_results$lowerCL[[2]], 3),  " to ", round(table_results$upperCL[[2]], 3), "]", "\n", "[95% PI = ",
round(table_results$lowerPR[[2]], 3), " to ",
round(table_results$upperPR[[2]], 3), "]")) +
...
In this section, we extractd the estimates (median, mean and 95% CIs) from lm and lmer model, both for experiment level and meta-analysis level 
```{r}
# meta-analysis level
# power
...
Table 2 = power for experiment and meta-analysis levels
Table 3 = Type S for experiment and meta-analysis levels
Table 4 = Type M for experiment and meta-analysis levels

```{r}
# table for experiment level power
...
# table for experiment level Type S
...
# table for experiment level Type M
...
### Heatmaps - power
In this section, we made heatmaps to show results for statistical power (Figure 3)
...
### Heatmaps - Type M
In this section, we made heatmaps to show results for Type M (Figure 5)
...
### E-O power
In this section, we made forest plots to show power differences in experiments and observations (Figure 6A - 6F)
...
### E-O Type M
In this section, we made forest plots to show Type M error differences in experiments and observations (Figure 6G - 6L)
```{r}
# MAOM
...
### E-O Type S
In this section, we made forest plots to show Type S error differences in experiments and observations (Figure 6M - 6R)
...
### Forest plots 
In this section, we made forest plosts (Figures in supplementary materials) to show meta-analytic means and their bias-corrected verison for each stressor (meta-analytic case). 
...
### Funnel plots - SE
Funnel plots reported in Supplementary materials
...
# Additional analysis
This section contains three additional analyses: (i) 'stressor specific' results, which are the additional analysis required by one of Referee, (ii) the empirically-derived effect size interpretation guidelines in global change studies, and (3) false-positive report probability (FPRP) in global change studies. We did not report these results in the maintext and online supplements because our datasets were not well balanced. However, given the importantce of the (i) - (iii), we We provid all our code that can reproduce the (i) - (iii). One can easily update our results if one has a “bigger” dataset. 


## Additional analysis 1
#### Stressor-specific results
Obviously, 'stressor specific' results are attractive and useful to ecologists and policymakers. Technically, to estimate 'stressor specific' results is not difficult – just incorporate the 'stressor types' as a moderator (a predictor or fixed effect) in a meta-regression model and estimate the corresponding slopes for each type of stressor. However, when submitting the manuscript, we decided not to provide 'stressor specific' results in the main text for two reasons: (1) we are not sure whether 'stressor specific' estimates are statistically reliable because of the unbalanced sample sizes for each stressor category, and (2) ecologists and even policymakers could misuse such unreliable 'stressor specific' estimates if we provide them.

For the first reason, although our dataset contained 30 meta-analyses with many field experiments (3,847), the sample sizes (i.e., experiments) were not well balanced for each stressor within our dataset. That is, some specific stressors have a “good” number of sample sizes (e.g., 811 fertilization experiments, 790 global warming experiments; we reported the detailed numbers for each included stressors in section 2.2 (lines 222 – 233) in the revised manuscript), while others have a “poor” number (e.g., 138 precipitation experiments, 179 bush fire experiments). For the specific stressor with a “good” number of sample sizes, we can be more confident with the reliability of estimates of effect size, power, magnitude, and sign errors. However, for a “poor” number specific stressor, we are less confident with the corresponding estimates.

For the second reason, if we provide the estimates for specific stressor, for example, effect size estimates of global warming, acidification, and bush fire. Ecologists would treat them as ‘true’ effects to apply for grants and plan their experiments (e.g., power calculation). Also, policymaker might use such estimates to make corresponding policies or interventions to protect ecology system and environment. However, as said, the estimates for some specific stressors (those with ‘poor’ sample sizes) are less statistically reliable (they might be the true effect and also might not be true – because of the relatively small sample sizes).


Notably, if one has enough data for a specific stressor, one can easily modify our exemplar script to obtain the effect sizes, statistical power, magnitude, and sign errors for his specific stressor.
...
# include stressor category as a fixed effect 
MMA_power.F_SMD.stressor <- lmer(power.F_SMD ~ 1 + I(stressor_SMD) -1 + (1 | studyID_SMD), data = individual_est_SMD)
summary(MMA_power.F_SMD.stressor)$coefficients
# log
MMA_power.F_SMD.stressor2 <- lmer(log(power.F_SMD) ~ 1 + I(stressor_SMD) -1 + (1 | studyID_SMD), data = individual_est_SMD)
# this is median
summary(MMA_power.F_SMD.stressor2)$coefficients %>% exp()


#---------------------adjusted_SMD ---------------------#

# include stressor category as a fixed effect 
MMA_power.F_adjusted_SMD.stressor <- lmer(power.F_adjusted_SMD ~ 1 + I(stressor_adjusted_SMD) -1 + (1 | studyID_adjusted_SMD), data = individual_est_adjusted_SMD)
summary(MMA_power.F_adjusted_SMD.stressor)$coefficients
# log
MMA_power.F_adjusted_SMD.stressor2 <- lmer(log(power.F_adjusted_SMD) ~ 1 + I(stressor_adjusted_SMD) -1 + (1 | studyID_adjusted_SMD), data = individual_est_adjusted_SMD)
# this is median
summary(MMA_power.F_adjusted_SMD.stressor2)$coefficients %>% exp()
...
# include stressor category as a fixed effect 
MMA_power.F_lnrr.stressor <- lmer(power.F_lnrr ~ 1 + I(stressor_lnrr) -1 + (1 | studyID_lnrr), data = individual_est_lnrr)
summary(MMA_power.F_lnrr.stressor)$coefficients
# log
MMA_power.F_lnrr.stressor2 <- lmer(log(power.F_lnrr) ~ 1 + I(stressor_lnrr) -1 + (1 | studyID_lnrr), data = individual_est_lnrr)
# this is median
summary(MMA_power.F_lnrr.stressor2)$coefficients %>% exp()


#---------------------adjusted_lnrr ---------------------#

# include stressor category as a fixed effect 
MMA_power.F_adjusted_lnrr.stressor <- lmer(power.F_adjusted_lnrr ~ 1 + I(stressor_adjusted_lnrr) -1 + (1 | studyID_adjusted_lnrr), data = individual_est_adjusted_lnrr)
summary(MMA_power.F_adjusted_lnrr.stressor)$coefficients
# log
MMA_power.F_adjusted_lnrr.stressor2 <- lmer(log(power.F_adjusted_lnrr) ~ 1 + I(stressor_adjusted_lnrr) -1 + (1 | studyID_adjusted_lnrr), data = individual_est_adjusted_lnrr)
# this is median
summary(MMA_power.F_adjusted_lnrr.stressor2)$coefficients %>% exp()
...
# include stressor category as a fixed effect 
MMA_power.F_SMDH.stressor <- lmer(power.F_SMDH ~ 1 + I(stressor_SMDH) -1 + (1 | studyID_SMDH), data = individual_est_SMDH)
summary(MMA_power.F_SMDH.stressor)$coefficients
# log
MMA_power.F_SMDH.stressor2 <- lmer(log(power.F_SMDH) ~ 1 + I(stressor_SMDH) -1 + (1 | studyID_SMDH), data = individual_est_SMDH)
# this is median
summary(MMA_power.F_SMDH.stressor2)$coefficients %>% exp()


#---------------------adjusted_SMDH ---------------------#

# include stressor category as a fixed effect 
MMA_power.F_adjusted_SMDH.stressor <- lmer(power.F_adjusted_SMDH ~ 1 + I(stressor_adjusted_SMDH) -1 + (1 | studyID_adjusted_SMDH), data = individual_est_adjusted_SMDH)
summary(MMA_power.F_adjusted_SMDH.stressor)$coefficients
# log
MMA_power.F_adjusted_SMDH.stressor2 <- lmer(log(power.F_adjusted_SMDH) ~ 1 + I(stressor_adjusted_SMDH) -1 + (1 | studyID_adjusted_SMDH), data = individual_est_adjusted_SMDH)
# this is median
summary(MMA_power.F_adjusted_SMDH.stressor2)$coefficients %>% exp()
...
# include stressor category as a fixed effect 
MMA_power.F_RR.stressor <- lmer(power.F_RR ~ 1 + I(stressor_RR) -1 + (1 | studyID_RR), data = individual_est_RR)
summary(MMA_power.F_RR.stressor)$coefficients
# log
MMA_power.F_RR.stressor2 <- lmer(log(power.F_RR) ~ 1 + I(stressor_RR) -1 + (1 | studyID_RR), data = individual_est_RR)
# this is median
summary(MMA_power.F_RR.stressor2)$coefficients %>% exp()


#---------------------adjusted_lnRR* ---------------------#

# include stressor category as a fixed effect 
MMA_power.F_adjusted_RR.stressor <- lmer(power.F_adjusted_RR ~ 1 + I(stressor_adjusted_RR) -1 + (1 | studyID_adjusted_RR), data = individual_est_adjusted_RR)
summary(MMA_power.F_adjusted_RR.stressor)$coefficients
# log
MMA_power.F_adjusted_RR.stressor2 <- lmer(log(power.F_adjusted_RR) ~ 1 + I(stressor_adjusted_RR) -1 + (1 | studyID_adjusted_RR), data = individual_est_adjusted_RR)
# this is median
summary(MMA_power.F_adjusted_RR.stressor2)$coefficients %>% exp()
...
# include stressor category as a fixed effect 
MMA_power.F_CVR.stressor <- lmer(power.F_CVR ~ 1 + I(stressor_CVR) -1 + (1 | studyID_CVR), data = individual_est_CVR)
summary(MMA_power.F_CVR.stressor)$coefficients
# log
MMA_power.F_CVR.stressor2 <- lmer(log(power.F_CVR) ~ 1 + I(stressor_CVR) -1 + (1 | studyID_CVR), data = individual_est_CVR)
# this is median
summary(MMA_power.F_CVR.stressor2)$coefficients %>% exp()
...
# include stressor category as a fixed effect 
MMA_power.F_VR.stressor <- lmer(power.F_VR ~ 1 + I(stressor_VR) -1 + (1 | studyID_VR), data = individual_est_VR)
summary(MMA_power.F_VR.stressor)$coefficients
# log
MMA_power.F_VR.stressor2 <- lmer(log(power.F_VR) ~ 1 + I(stressor_VR) -1 + (1 | studyID_VR), data = individual_est_VR)
# this is median
summary(MMA_power.F_VR.stressor2)$coefficients %>% exp()
...
# include stressor category as a fixed effect 
MMA_power.R_SMD.stressor <- lmer(power.R_SMD ~ 1 + I(stressor_SMD) -1 + (1 | studyID_SMD), data = individual_est_SMD)
summary(MMA_power.R_SMD.stressor)$coefficients
# log
MMA_power.R_SMD.stressor2 <- lmer(log(power.R_SMD) ~ 1 + I(stressor_SMD) -1 + (1 | studyID_SMD), data = individual_est_SMD)
# this is median
summary(MMA_power.R_SMD.stressor2)$coefficients %>% exp()


#---------------------adjusted_SMD ---------------------#

# include stressor category as a fixed effect 
MMA_power.R_adjusted_SMD.stressor <- lmer(power.R_adjusted_SMD ~ 1 + I(stressor_adjusted_SMD) -1 + (1 | studyID_adjusted_SMD), data = individual_est_adjusted_SMD)
summary(MMA_power.R_adjusted_SMD.stressor)$coefficients
# log
MMA_power.R_adjusted_SMD.stressor2 <- lmer(log(power.R_adjusted_SMD) ~ 1 + I(stressor_adjusted_SMD) -1 + (1 | studyID_adjusted_SMD), data = individual_est_adjusted_SMD)
# this is median
summary(MMA_power.R_adjusted_SMD.stressor2)$coefficients %>% exp()
...
# include stressor category as a fixed effect 
MMA_power.R_lnrr.stressor <- lmer(power.R_lnrr ~ 1 + I(stressor_lnrr) -1 + (1 | studyID_lnrr), data = individual_est_lnrr)
summary(MMA_power.R_lnrr.stressor)$coefficients
# log
MMA_power.R_lnrr.stressor2 <- lmer(log(power.R_lnrr) ~ 1 + I(stressor_lnrr) -1 + (1 | studyID_lnrr), data = individual_est_lnrr)
# this is median
summary(MMA_power.R_lnrr.stressor2)$coefficients %>% exp()


#---------------------adjusted_lnrr ---------------------#

# include stressor category as a fixed effect 
MMA_power.R_adjusted_lnrr.stressor <- lmer(power.R_adjusted_lnrr ~ 1 + I(stressor_adjusted_lnrr) -1 + (1 | studyID_adjusted_lnrr), data = individual_est_adjusted_lnrr)
summary(MMA_power.R_adjusted_lnrr.stressor)$coefficients
# log
MMA_power.R_adjusted_lnrr.stressor2 <- lmer(log(power.R_adjusted_lnrr) ~ 1 + I(stressor_adjusted_lnrr) -1 + (1 | studyID_adjusted_lnrr), data = individual_est_adjusted_lnrr)
# this is median
summary(MMA_power.R_adjusted_lnrr.stressor2)$coefficients %>% exp()
...
# include stressor category as a fixed effect 
MMA_power.R_SMDH.stressor <- lmer(power.R_SMDH ~ 1 + I(stressor_SMDH) -1 + (1 | studyID_SMDH), data = individual_est_SMDH)
summary(MMA_power.R_SMDH.stressor)$coefficients
# log
MMA_power.R_SMDH.stressor2 <- lmer(log(power.R_SMDH) ~ 1 + I(stressor_SMDH) -1 + (1 | studyID_SMDH), data = individual_est_SMDH)
# this is median
summary(MMA_power.R_SMDH.stressor2)$coefficients %>% exp()


#---------------------adjusted_SMDH ---------------------#

# include stressor category as a fixed effect 
MMA_power.R_adjusted_SMDH.stressor <- lmer(power.R_adjusted_SMDH ~ 1 + I(stressor_adjusted_SMDH) -1 + (1 | studyID_adjusted_SMDH), data = individual_est_adjusted_SMDH)
summary(MMA_power.R_adjusted_SMDH.stressor)$coefficients
# log
MMA_power.R_adjusted_SMDH.stressor2 <- lmer(log(power.R_adjusted_SMDH) ~ 1 + I(stressor_adjusted_SMDH) -1 + (1 | studyID_adjusted_SMDH), data = individual_est_adjusted_SMDH)
# this is median
summary(MMA_power.R_adjusted_SMDH.stressor2)$coefficients %>% exp()
...
# include stressor category as a fixed effect 
MMA_power.R_RR.stressor <- lmer(power.R_RR ~ 1 + I(stressor_RR) -1 + (1 | studyID_RR), data = individual_est_RR)
summary(MMA_power.R_RR.stressor)$coefficients
# log
MMA_power.R_RR.stressor2 <- lmer(log(power.R_RR) ~ 1 + I(stressor_RR) -1 + (1 | studyID_RR), data = individual_est_RR)
# this is median
summary(MMA_power.R_RR.stressor2)$coefficients %>% exp()


#---------------------adjusted_lnRR* ---------------------#

# include stressor category as a fixed effect 
MMA_power.R_adjusted_RR.stressor <- lmer(power.R_adjusted_RR ~ 1 + I(stressor_adjusted_RR) -1 + (1 | studyID_adjusted_RR), data = individual_est_adjusted_RR)
summary(MMA_power.R_adjusted_RR.stressor)$coefficients
# log
MMA_power.R_adjusted_RR.stressor2 <- lmer(log(power.R_adjusted_RR) ~ 1 + I(stressor_adjusted_RR) -1 + (1 | studyID_adjusted_RR), data = individual_est_adjusted_RR)
# this is median
summary(MMA_power.R_adjusted_RR.stressor2)$coefficients %>% exp()
...
# include stressor category as a fixed effect 
MMA_power.R_CVR.stressor <- lmer(power.R_CVR ~ 1 + I(stressor_CVR) -1 + (1 | studyID_CVR), data = individual_est_CVR)
summary(MMA_power.R_CVR.stressor)$coefficients
# log
MMA_power.R_CVR.stressor2 <- lmer(log(power.R_CVR) ~ 1 + I(stressor_CVR) -1 + (1 | studyID_CVR), data = individual_est_CVR)
# this is median
summary(MMA_power.R_CVR.stressor2)$coefficients %>% exp()
...
# include stressor category as a fixed effect 
MMA_power.R_VR.stressor <- lmer(power.R_VR ~ 1 + I(stressor_VR) -1 + (1 | studyID_VR), data = individual_est_VR)
summary(MMA_power.R_VR.stressor)$coefficients
# log
MMA_power.R_VR.stressor2 <- lmer(log(power.R_VR) ~ 1 + I(stressor_VR) -1 + (1 | studyID_VR), data = individual_est_VR)
# this is median
summary(MMA_power.R_VR.stressor2)$coefficients %>% exp()
...
# include stressor category as a fixed effect 
MMA_power.F.S_SMD.stressor <- lmer(power.F.S_SMD ~ 1 + I(stressor_SMD) -1 + (1 | studyID_SMD), data = individual_est_SMD)
summary(MMA_power.F.S_SMD.stressor)$coefficients
# log
MMA_power.F.S_SMD.stressor2 <- lmer(log(power.F.S_SMD+ 0.025) ~ 1 + I(stressor_SMD) -1 + (1 | studyID_SMD), data = individual_est_SMD)
# this is median
summary(MMA_power.F.S_SMD.stressor2)$coefficients %>% exp() - 0.025


#---------------------adjusted_SMD ---------------------#

# include stressor category as a fixed effect 
MMA_power.F.S_adjusted_SMD.stressor <- lmer(power.F.S_adjusted_SMD ~ 1 + I(stressor_adjusted_SMD) -1 + (1 | studyID_adjusted_SMD), data = individual_est_adjusted_SMD)
summary(MMA_power.F.S_adjusted_SMD.stressor)$coefficients
# log
MMA_power.F.S_adjusted_SMD.stressor2 <- lmer(log(power.F.S_adjusted_SMD+ 0.025) ~ 1 + I(stressor_adjusted_SMD) -1 + (1 | studyID_adjusted_SMD), data = individual_est_adjusted_SMD)
# this is median
summary(MMA_power.F.S_adjusted_SMD.stressor2)$coefficients %>% exp() - 0.025
...
# include stressor category as a fixed effect 
MMA_power.F.S_lnrr.stressor <- lmer(power.F.S_lnrr ~ 1 + I(stressor_lnrr) -1 + (1 | studyID_lnrr), data = individual_est_lnrr)
summary(MMA_power.F.S_lnrr.stressor)$coefficients
# log
MMA_power.F.S_lnrr.stressor2 <- lmer(log(power.F.S_lnrr+ 0.025) ~ 1 + I(stressor_lnrr) -1 + (1 | studyID_lnrr), data = individual_est_lnrr)
# this is median
summary(MMA_power.F.S_lnrr.stressor2)$coefficients %>% exp() - 0.025


#---------------------adjusted_lnrr ---------------------#

# include stressor category as a fixed effect 
MMA_power.F.S_adjusted_lnrr.stressor <- lmer(power.F.S_adjusted_lnrr ~ 1 + I(stressor_adjusted_lnrr) -1 + (1 | studyID_adjusted_lnrr), data = individual_est_adjusted_lnrr)
summary(MMA_power.F.S_adjusted_lnrr.stressor)$coefficients
# log
MMA_power.F.S_adjusted_lnrr.stressor2 <- lmer(log(power.F.S_adjusted_lnrr+ 0.025) ~ 1 + I(stressor_adjusted_lnrr) -1 + (1 | studyID_adjusted_lnrr), data = individual_est_adjusted_lnrr)
# this is median
summary(MMA_power.F.S_adjusted_lnrr.stressor2)$coefficients %>% exp() - 0.025
...
# include stressor category as a fixed effect 
MMA_power.F.S_SMDH.stressor <- lmer(power.F.S_SMDH ~ 1 + I(stressor_SMDH) -1 + (1 | studyID_SMDH), data = individual_est_SMDH)
summary(MMA_power.F.S_SMDH.stressor)$coefficients
# log
MMA_power.F.S_SMDH.stressor2 <- lmer(log(power.F.S_SMDH+ 0.025) ~ 1 + I(stressor_SMDH) -1 + (1 | studyID_SMDH), data = individual_est_SMDH)
# this is median
summary(MMA_power.F.S_SMDH.stressor2)$coefficients %>% exp() - 0.025


#---------------------adjusted_SMDH ---------------------#

# include stressor category as a fixed effect 
MMA_power.F.S_adjusted_SMDH.stressor <- lmer(power.F.S_adjusted_SMDH ~ 1 + I(stressor_adjusted_SMDH) -1 + (1 | studyID_adjusted_SMDH), data = individual_est_adjusted_SMDH)
summary(MMA_power.F.S_adjusted_SMDH.stressor)$coefficients
# log
MMA_power.F.S_adjusted_SMDH.stressor2 <- lmer(log(power.F.S_adjusted_SMDH+ 0.025) ~ 1 + I(stressor_adjusted_SMDH) -1 + (1 | studyID_adjusted_SMDH), data = individual_est_adjusted_SMDH)
# this is median
summary(MMA_power.F.S_adjusted_SMDH.stressor2)$coefficients %>% exp() - 0.025
...
# include stressor category as a fixed effect 
MMA_power.F.S_RR.stressor <- lmer(power.F.S_RR ~ 1 + I(stressor_RR) -1 + (1 | studyID_RR), data = individual_est_RR)
summary(MMA_power.F.S_RR.stressor)$coefficients
# log
MMA_power.F.S_RR.stressor2 <- lmer(log(power.F.S_RR+ 0.025) ~ 1 + I(stressor_RR) -1 + (1 | studyID_RR), data = individual_est_RR)
# this is median
summary(MMA_power.F.S_RR.stressor2)$coefficients %>% exp() - 0.025


#---------------------adjusted_lnRR* ---------------------#

# include stressor category as a fixed effect 
MMA_power.F.S_adjusted_RR.stressor <- lmer(power.F.S_adjusted_RR ~ 1 + I(stressor_adjusted_RR) -1 + (1 | studyID_adjusted_RR), data = individual_est_adjusted_RR)
summary(MMA_power.F.S_adjusted_RR.stressor)$coefficients
# log
MMA_power.F.S_adjusted_RR.stressor2 <- lmer(log(power.F.S_adjusted_RR+ 0.025) ~ 1 + I(stressor_adjusted_RR) -1 + (1 | studyID_adjusted_RR), data = individual_est_adjusted_RR)
# this is median
summary(MMA_power.F.S_adjusted_RR.stressor2)$coefficients %>% exp() - 0.025
...
# include stressor category as a fixed effect 
MMA_power.F.S_CVR.stressor <- lmer(power.F.S_CVR ~ 1 + I(stressor_CVR) -1 + (1 | studyID_CVR), data = individual_est_CVR)
summary(MMA_power.F.S_CVR.stressor)$coefficients
# log
MMA_power.F.S_CVR.stressor2 <- lmer(log(power.F.S_CVR+ 0.025) ~ 1 + I(stressor_CVR) -1 + (1 | studyID_CVR), data = individual_est_CVR)
# this is median
summary(MMA_power.F.S_CVR.stressor2)$coefficients %>% exp() - 0.025
...
# include stressor category as a fixed effect 
MMA_power.F.S_VR.stressor <- lmer(power.F.S_VR ~ 1 + I(stressor_VR) -1 + (1 | studyID_VR), data = individual_est_VR)
summary(MMA_power.F.S_VR.stressor)$coefficients
# log
MMA_power.F.S_VR.stressor2 <- lmer(log(power.F.S_VR+ 0.025) ~ 1 + I(stressor_VR) -1 + (1 | studyID_VR), data = individual_est_VR)
# this is median
summary(MMA_power.F.S_VR.stressor2)$coefficients %>% exp() - 0.025
...
# include stressor category as a fixed effect 
MMA_power.R.S_SMD.stressor <- lmer(power.R.S_SMD ~ 1 + I(stressor_SMD) -1 + (1 | studyID_SMD), data = individual_est_SMD)
summary(MMA_power.R.S_SMD.stressor)$coefficients
# log
MMA_power.R.S_SMD.stressor2 <- lmer(log(power.R.S_SMD+ 0.025) ~ 1 + I(stressor_SMD) -1 + (1 | studyID_SMD), data = individual_est_SMD)
# this is median
summary(MMA_power.R.S_SMD.stressor2)$coefficients %>% exp() - 0.025


#---------------------adjusted_SMD ---------------------#

# include stressor category as a fixed effect 
MMA_power.R.S_adjusted_SMD.stressor <- lmer(power.R.S_adjusted_SMD ~ 1 + I(stressor_adjusted_SMD) -1 + (1 | studyID_adjusted_SMD), data = individual_est_adjusted_SMD)
summary(MMA_power.R.S_adjusted_SMD.stressor)$coefficients
# log
MMA_power.R.S_adjusted_SMD.stressor2 <- lmer(log(power.R.S_adjusted_SMD+ 0.025) ~ 1 + I(stressor_adjusted_SMD) -1 + (1 | studyID_adjusted_SMD), data = individual_est_adjusted_SMD)
# this is median
summary(MMA_power.R.S_adjusted_SMD.stressor2)$coefficients %>% exp() - 0.025
...
# include stressor category as a fixed effect 
MMA_power.R.S_lnrr.stressor <- lmer(power.R.S_lnrr ~ 1 + I(stressor_lnrr) -1 + (1 | studyID_lnrr), data = individual_est_lnrr)
summary(MMA_power.R.S_lnrr.stressor)$coefficients
# log
MMA_power.R.S_lnrr.stressor2 <- lmer(log(power.R.S_lnrr+ 0.025) ~ 1 + I(stressor_lnrr) -1 + (1 | studyID_lnrr), data = individual_est_lnrr)
# this is median
summary(MMA_power.R.S_lnrr.stressor2)$coefficients %>% exp() - 0.025


#---------------------adjusted_lnrr ---------------------#

# include stressor category as a fixed effect 
MMA_power.R.S_adjusted_lnrr.stressor <- lmer(power.R.S_adjusted_lnrr ~ 1 + I(stressor_adjusted_lnrr) -1 + (1 | studyID_adjusted_lnrr), data = individual_est_adjusted_lnrr)
summary(MMA_power.R.S_adjusted_lnrr.stressor)$coefficients
# log
MMA_power.R.S_adjusted_lnrr.stressor2 <- lmer(log(power.R.S_adjusted_lnrr+ 0.025) ~ 1 + I(stressor_adjusted_lnrr) -1 + (1 | studyID_adjusted_lnrr), data = individual_est_adjusted_lnrr)
# this is median
summary(MMA_power.R.S_adjusted_lnrr.stressor2)$coefficients %>% exp() - 0.025
...
# include stressor category as a fixed effect 
MMA_power.R.S_SMDH.stressor <- lmer(power.R.S_SMDH ~ 1 + I(stressor_SMDH) -1 + (1 | studyID_SMDH), data = individual_est_SMDH)
summary(MMA_power.R.S_SMDH.stressor)$coefficients
# log
MMA_power.R.S_SMDH.stressor2 <- lmer(log(power.R.S_SMDH+ 0.025) ~ 1 + I(stressor_SMDH) -1 + (1 | studyID_SMDH), data = individual_est_SMDH)
# this is median
summary(MMA_power.R.S_SMDH.stressor2)$coefficients %>% exp() - 0.025


#---------------------adjusted_SMDH ---------------------#

# include stressor category as a fixed effect 
MMA_power.R.S_adjusted_SMDH.stressor <- lmer(power.R.S_adjusted_SMDH ~ 1 + I(stressor_adjusted_SMDH) -1 + (1 | studyID_adjusted_SMDH), data = individual_est_adjusted_SMDH)
summary(MMA_power.R.S_adjusted_SMDH.stressor)$coefficients
# log
MMA_power.R.S_adjusted_SMDH.stressor2 <- lmer(log(power.R.S_adjusted_SMDH+ 0.025) ~ 1 + I(stressor_adjusted_SMDH) -1 + (1 | studyID_adjusted_SMDH), data = individual_est_adjusted_SMDH)
# this is median
summary(MMA_power.R.S_adjusted_SMDH.stressor2)$coefficients %>% exp() - 0.025
...
# include stressor category as a fixed effect 
MMA_power.R.S_RR.stressor <- lmer(power.R.S_RR ~ 1 + I(stressor_RR) -1 + (1 | studyID_RR), data = individual_est_RR)
summary(MMA_power.R.S_RR.stressor)$coefficients
# log
MMA_power.R.S_RR.stressor2 <- lmer(log(power.R.S_RR+ 0.025) ~ 1 + I(stressor_RR) -1 + (1 | studyID_RR), data = individual_est_RR)
# this is median
summary(MMA_power.R.S_RR.stressor2)$coefficients %>% exp() - 0.025


#---------------------adjusted_lnRR* ---------------------#

# include stressor category as a fixed effect 
MMA_power.R.S_adjusted_RR.stressor <- lmer(power.R.S_adjusted_RR ~ 1 + I(stressor_adjusted_RR) -1 + (1 | studyID_adjusted_RR), data = individual_est_adjusted_RR)
summary(MMA_power.R.S_adjusted_RR.stressor)$coefficients
# log
MMA_power.R.S_adjusted_RR.stressor2 <- lmer(log(power.R.S_adjusted_RR+ 0.025) ~ 1 + I(stressor_adjusted_RR) -1 + (1 | studyID_adjusted_RR), data = individual_est_adjusted_RR)
# this is median
summary(MMA_power.R.S_adjusted_RR.stressor2)$coefficients %>% exp() - 0.025
...
# include stressor category as a fixed effect 
MMA_power.R.S_CVR.stressor <- lmer(power.R.S_CVR ~ 1 + I(stressor_CVR) -1 + (1 | studyID_CVR), data = individual_est_CVR)
summary(MMA_power.R.S_CVR.stressor)$coefficients
# log
MMA_power.R.S_CVR.stressor2 <- lmer(log(power.R.S_CVR+ 0.025) ~ 1 + I(stressor_CVR) -1 + (1 | studyID_CVR), data = individual_est_CVR)
# this is median
summary(MMA_power.R.S_CVR.stressor2)$coefficients %>% exp() - 0.025
...
# include stressor category as a fixed effect 
MMA_power.R.S_VR.stressor <- lmer(power.R.S_VR ~ 1 + I(stressor_VR) -1 + (1 | studyID_VR), data = individual_est_VR)
summary(MMA_power.R.S_VR.stressor)$coefficients
# log
MMA_power.R.S_VR.stressor2 <- lmer(log(power.R.S_VR+ 0.025) ~ 1 + I(stressor_VR) -1 + (1 | studyID_VR), data = individual_est_VR)
# this is median
summary(MMA_power.R.S_VR.stressor2)$coefficients %>% exp() - 0.025
...
# include stressor category as a fixed effect 
MMA_power.F.M_SMD.stressor <- lmer(power.F.M_SMD ~ 1 + I(stressor_SMD) -1 + (1 | studyID_SMD), data = individual_est_SMD)
summary(MMA_power.F.M_SMD.stressor)$coefficients
# log
MMA_power.F.M_SMD.stressor2 <- lmer(log(power.F.M_SMD+ 0.025) ~ 1 + I(stressor_SMD) -1 + (1 | studyID_SMD), data = individual_est_SMD)
# this is median
summary(MMA_power.F.M_SMD.stressor2)$coefficients %>% exp() - 0.025


#---------------------adjusted_SMD ---------------------#

# include stressor category as a fixed effect 
MMA_power.F.M_adjusted_SMD.stressor <- lmer(power.F.M_adjusted_SMD ~ 1 + I(stressor_adjusted_SMD) -1 + (1 | studyID_adjusted_SMD), data = individual_est_adjusted_SMD)
summary(MMA_power.F.M_adjusted_SMD.stressor)$coefficients
# log
MMA_power.F.M_adjusted_SMD.stressor2 <- lmer(log(power.F.M_adjusted_SMD+ 0.025) ~ 1 + I(stressor_adjusted_SMD) -1 + (1 | studyID_adjusted_SMD), data = individual_est_adjusted_SMD)
# this is median
summary(MMA_power.F.M_adjusted_SMD.stressor2)$coefficients %>% exp() - 0.025
...
# include stressor category as a fixed effect 
MMA_power.F.M_lnrr.stressor <- lmer(power.F.M_lnrr ~ 1 + I(stressor_lnrr) -1 + (1 | studyID_lnrr), data = individual_est_lnrr)
summary(MMA_power.F.M_lnrr.stressor)$coefficients
# log
MMA_power.F.M_lnrr.stressor2 <- lmer(log(power.F.M_lnrr+ 0.025) ~ 1 + I(stressor_lnrr) -1 + (1 | studyID_lnrr), data = individual_est_lnrr)
# this is median
summary(MMA_power.F.M_lnrr.stressor2)$coefficients %>% exp() - 0.025


#---------------------adjusted_lnrr ---------------------#

# include stressor category as a fixed effect 
MMA_power.F.M_adjusted_lnrr.stressor <- lmer(power.F.M_adjusted_lnrr ~ 1 + I(stressor_adjusted_lnrr) -1 + (1 | studyID_adjusted_lnrr), data = individual_est_adjusted_lnrr)
summary(MMA_power.F.M_adjusted_lnrr.stressor)$coefficients
# log
MMA_power.F.M_adjusted_lnrr.stressor2 <- lmer(log(power.F.M_adjusted_lnrr+ 0.025) ~ 1 + I(stressor_adjusted_lnrr) -1 + (1 | studyID_adjusted_lnrr), data = individual_est_adjusted_lnrr)
# this is median
summary(MMA_power.F.M_adjusted_lnrr.stressor2)$coefficients %>% exp() - 0.025
...
# include stressor category as a fixed effect 
MMA_power.F.M_SMDH.stressor <- lmer(power.F.M_SMDH ~ 1 + I(stressor_SMDH) -1 + (1 | studyID_SMDH), data = individual_est_SMDH)
summary(MMA_power.F.M_SMDH.stressor)$coefficients
# log
MMA_power.F.M_SMDH.stressor2 <- lmer(log(power.F.M_SMDH+ 0.025) ~ 1 + I(stressor_SMDH) -1 + (1 | studyID_SMDH), data = individual_est_SMDH)
# this is median
summary(MMA_power.F.M_SMDH.stressor2)$coefficients %>% exp() - 0.025


#---------------------adjusted_SMDH ---------------------#

# include stressor category as a fixed effect 
MMA_power.F.M_adjusted_SMDH.stressor <- lmer(power.F.M_adjusted_SMDH ~ 1 + I(stressor_adjusted_SMDH) -1 + (1 | studyID_adjusted_SMDH), data = individual_est_adjusted_SMDH)
summary(MMA_power.F.M_adjusted_SMDH.stressor)$coefficients
# log
MMA_power.F.M_adjusted_SMDH.stressor2 <- lmer(log(power.F.M_adjusted_SMDH+ 0.025) ~ 1 + I(stressor_adjusted_SMDH) -1 + (1 | studyID_adjusted_SMDH), data = individual_est_adjusted_SMDH)
# this is median
summary(MMA_power.F.M_adjusted_SMDH.stressor2)$coefficients %>% exp() - 0.025
...
# include stressor category as a fixed effect 
MMA_power.F.M_RR.stressor <- lmer(power.F.M_RR ~ 1 + I(stressor_RR) -1 + (1 | studyID_RR), data = individual_est_RR)
summary(MMA_power.F.M_RR.stressor)$coefficients
# log
MMA_power.F.M_RR.stressor2 <- lmer(log(power.F.M_RR+ 0.025) ~ 1 + I(stressor_RR) -1 + (1 | studyID_RR), data = individual_est_RR)
# this is median
summary(MMA_power.F.M_RR.stressor2)$coefficients %>% exp() - 0.025


#---------------------adjusted_lnRR* ---------------------#

# include stressor category as a fixed effect 
MMA_power.F.M_adjusted_RR.stressor <- lmer(power.F.M_adjusted_RR ~ 1 + I(stressor_adjusted_RR) -1 + (1 | studyID_adjusted_RR), data = individual_est_adjusted_RR)
summary(MMA_power.F.M_adjusted_RR.stressor)$coefficients
# log
MMA_power.F.M_adjusted_RR.stressor2 <- lmer(log(power.F.M_adjusted_RR+ 0.025) ~ 1 + I(stressor_adjusted_RR) -1 + (1 | studyID_adjusted_RR), data = individual_est_adjusted_RR)
# this is median
summary(MMA_power.F.M_adjusted_RR.stressor2)$coefficients %>% exp() - 0.025
...
# include stressor category as a fixed effect 
MMA_power.F.M_CVR.stressor <- lmer(power.F.M_CVR ~ 1 + I(stressor_CVR) -1 + (1 | studyID_CVR), data = individual_est_CVR)
summary(MMA_power.F.M_CVR.stressor)$coefficients
# log
MMA_power.F.M_CVR.stressor2 <- lmer(log(power.F.M_CVR+ 0.025) ~ 1 + I(stressor_CVR) -1 + (1 | studyID_CVR), data = individual_est_CVR)
# this is median
summary(MMA_power.F.M_CVR.stressor2)$coefficients %>% exp() - 0.025
...
# include stressor category as a fixed effect 
MMA_power.F.M_VR.stressor <- lmer(power.F.M_VR ~ 1 + I(stressor_VR) -1 + (1 | studyID_VR), data = individual_est_VR)
summary(MMA_power.F.M_VR.stressor)$coefficients
# log
MMA_power.F.M_VR.stressor2 <- lmer(log(power.F.M_VR+ 0.025) ~ 1 + I(stressor_VR) -1 + (1 | studyID_VR), data = individual_est_VR)
# this is median
summary(MMA_power.F.M_VR.stressor2)$coefficients %>% exp() - 0.025
...
# include stressor category as a fixed effect 
MMA_power.R.M_SMD.stressor <- lmer(power.R.M_SMD ~ 1 + I(stressor_SMD) -1 + (1 | studyID_SMD), data = individual_est_SMD)
summary(MMA_power.R.M_SMD.stressor)$coefficients
# log
MMA_power.R.M_SMD.stressor2 <- lmer(log(power.R.M_SMD+ 0.025) ~ 1 + I(stressor_SMD) -1 + (1 | studyID_SMD), data = individual_est_SMD)
# this is median
summary(MMA_power.R.M_SMD.stressor2)$coefficients %>% exp() - 0.025


#---------------------adjusted_SMD ---------------------#

# include stressor category as a fixed effect 
MMA_power.R.M_adjusted_SMD.stressor <- lmer(power.R.M_adjusted_SMD ~ 1 + I(stressor_adjusted_SMD) -1 + (1 | studyID_adjusted_SMD), data = individual_est_adjusted_SMD)
summary(MMA_power.R.M_adjusted_SMD.stressor)$coefficients
# log
MMA_power.R.M_adjusted_SMD.stressor2 <- lmer(log(power.R.M_adjusted_SMD+ 0.025) ~ 1 + I(stressor_adjusted_SMD) -1 + (1 | studyID_adjusted_SMD), data = individual_est_adjusted_SMD)
# this is median
summary(MMA_power.R.M_adjusted_SMD.stressor2)$coefficients %>% exp() - 0.025
...
# include stressor category as a fixed effect 
MMA_power.R.M_lnrr.stressor <- lmer(power.R.M_lnrr ~ 1 + I(stressor_lnrr) -1 + (1 | studyID_lnrr), data = individual_est_lnrr)
summary(MMA_power.R.M_lnrr.stressor)$coefficients
# log
MMA_power.R.M_lnrr.stressor2 <- lmer(log(power.R.M_lnrr+ 0.025) ~ 1 + I(stressor_lnrr) -1 + (1 | studyID_lnrr), data = individual_est_lnrr)
# this is median
summary(MMA_power.R.M_lnrr.stressor2)$coefficients %>% exp() - 0.025


#---------------------adjusted_lnrr ---------------------#

# include stressor category as a fixed effect 
MMA_power.R.M_adjusted_lnrr.stressor <- lmer(power.R.M_adjusted_lnrr ~ 1 + I(stressor_adjusted_lnrr) -1 + (1 | studyID_adjusted_lnrr), data = individual_est_adjusted_lnrr)
summary(MMA_power.R.M_adjusted_lnrr.stressor)$coefficients
# log
MMA_power.R.M_adjusted_lnrr.stressor2 <- lmer(log(power.R.M_adjusted_lnrr+ 0.025) ~ 1 + I(stressor_adjusted_lnrr) -1 + (1 | studyID_adjusted_lnrr), data = individual_est_adjusted_lnrr)
# this is median
summary(MMA_power.R.M_adjusted_lnrr.stressor2)$coefficients %>% exp() - 0.025
...
# include stressor category as a fixed effect 
MMA_power.R.M_SMDH.stressor <- lmer(power.R.M_SMDH ~ 1 + I(stressor_SMDH) -1 + (1 | studyID_SMDH), data = individual_est_SMDH)
summary(MMA_power.R.M_SMDH.stressor)$coefficients
# log
MMA_power.R.M_SMDH.stressor2 <- lmer(log(power.R.M_SMDH+ 0.025) ~ 1 + I(stressor_SMDH) -1 + (1 | studyID_SMDH), data = individual_est_SMDH)
# this is median
summary(MMA_power.R.M_SMDH.stressor2)$coefficients %>% exp() - 0.025


#---------------------adjusted_SMDH ---------------------#

# include stressor category as a fixed effect 
MMA_power.R.M_adjusted_SMDH.stressor <- lmer(power.R.M_adjusted_SMDH ~ 1 + I(stressor_adjusted_SMDH) -1 + (1 | studyID_adjusted_SMDH), data = individual_est_adjusted_SMDH)
summary(MMA_power.R.M_adjusted_SMDH.stressor)$coefficients
# log
MMA_power.R.M_adjusted_SMDH.stressor2 <- lmer(log(power.R.M_adjusted_SMDH+ 0.025) ~ 1 + I(stressor_adjusted_SMDH) -1 + (1 | studyID_adjusted_SMDH), data = individual_est_adjusted_SMDH)
# this is median
summary(MMA_power.R.M_adjusted_SMDH.stressor2)$coefficients %>% exp() - 0.025
...
# include stressor category as a fixed effect 
MMA_power.R.M_RR.stressor <- lmer(power.R.M_RR ~ 1 + I(stressor_RR) -1 + (1 | studyID_RR), data = individual_est_RR)
summary(MMA_power.R.M_RR.stressor)$coefficients
# log
MMA_power.R.M_RR.stressor2 <- lmer(log(power.R.M_RR+ 0.025) ~ 1 + I(stressor_RR) -1 + (1 | studyID_RR), data = individual_est_RR)
# this is median
summary(MMA_power.R.M_RR.stressor2)$coefficients %>% exp() - 0.025


#---------------------adjusted_lnRR* ---------------------#

# include stressor category as a fixed effect 
MMA_power.R.M_adjusted_RR.stressor <- lmer(power.R.M_adjusted_RR ~ 1 + I(stressor_adjusted_RR) -1 + (1 | studyID_adjusted_RR), data = individual_est_adjusted_RR)
summary(MMA_power.R.M_adjusted_RR.stressor)$coefficients
# log
MMA_power.R.M_adjusted_RR.stressor2 <- lmer(log(power.R.M_adjusted_RR+ 0.025) ~ 1 + I(stressor_adjusted_RR) -1 + (1 | studyID_adjusted_RR), data = individual_est_adjusted_RR)
# this is median
summary(MMA_power.R.M_adjusted_RR.stressor2)$coefficients %>% exp() - 0.025
...
# include stressor category as a fixed effect 
MMA_power.R.M_CVR.stressor <- lmer(power.R.M_CVR ~ 1 + I(stressor_CVR) -1 + (1 | studyID_CVR), data = individual_est_CVR)
summary(MMA_power.R.M_CVR.stressor)$coefficients
# log
MMA_power.R.M_CVR.stressor2 <- lmer(log(power.R.M_CVR+ 0.025) ~ 1 + I(stressor_CVR) -1 + (1 | studyID_CVR), data = individual_est_CVR)
# this is median
summary(MMA_power.R.M_CVR.stressor2)$coefficients %>% exp() - 0.025
...
# include stressor category as a fixed effect 
MMA_power.R.M_VR.stressor <- lmer(power.R.M_VR ~ 1 + I(stressor_VR) -1 + (1 | studyID_VR), data = individual_est_VR)
summary(MMA_power.R.M_VR.stressor)$coefficients
# log
MMA_power.R.M_VR.stressor2 <- lmer(log(power.R.M_VR+ 0.025) ~ 1 + I(stressor_VR) -1 + (1 | studyID_VR), data = individual_est_VR)
# this is median
summary(MMA_power.R.M_VR.stressor2)$coefficients %>% exp() - 0.025
...
This section is to visualize the effect size distribution in global change studies. The results can be used as the empirically-derived effect-size interpretation guideline for global change stuides. However, our current datasets are not well balanced, so we did not report corresponding results in the main text. Interested researchers can easily modify the code to obtain the gudelines if one had a bigger dataset
...
This section is the additional analysis about false-positive report probability (FPRP). Computationally, FPRP is the number of statistically significant false positive findings divided by the total number of statistically significant findings. TRP is the number of statistically significant true positive findings divided by the total number of statistically significant findings
...
simulated_figS1_A <- plot_p_M_S(A = 0.1, s = 0.5, alpha = 0.05, df = Inf, n.sims = 1000) + labs(title = 'A - power = 5%, Type M = 12, Type S = 28% ', x = 'Replication (mean = 0.1, SD = 0.5)') +  annotate("rect", xmin = -Inf, ymin = -0.5*qt(1-0.05/2,df=10000), xmax = Inf, ymax = 0.5*qt(1-0.05/2,df=10000), fill = "green", alpha = 0.05)



simulated_figS1_B <- plot_p_M_S(A = 2, s = 0.5, alpha = 0.05, df = Inf, n.sims = 1000) + labs(title = 'B - power = 98%, Type M = 1, Type S = 0%', x = 'Replication (mean = 2, SD = 0.5)') + annotate("rect", xmin = -Inf, ymin = -0.5*qt(1-0.05/2,df=10000), xmax = Inf, ymax = 0.5*qt(1-0.05/2,df=10000), fill = "green", alpha = 0.05)


png(filename = "./Figures/simulated_figS1.png", width = 5, height = 10, units = "in", type = "windows", res = 400)
simulated_figS1_A + simulated_figS1_B + patchwork::plot_layout(nrow = 2 , ncol = 1)
dev.off()

# check the power, Type M and S errors for a normal distribution with meann 0.1 (or 2), and a variance of 0.5 - SMD assumed
retrodesign::retrodesign(A = 0.1, s = 0.5)
retrodesign::retrodesign(A = 2, s = 0.5)
```

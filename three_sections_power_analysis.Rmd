...
for (i in 1:length(power.F_RR)) {
  dat_list[[i]]$power.F_RR <- power_RR[[i]]
}
...
for (i in 1:length(power.M_RR)) {
  power.M_RR[i] <- mapply(unlist, power_M[[i]]) %>% data.frame()
} # convert into dataframe - early format contains column names
...
# delete se = 0 - some original datasets were lacking sampling errors. Helmut setted all of them to same value. So, se.mean will be 0
power.F_summary_RR2 <- power.F_summary_RR %>% subset(power.F_summary_RR$se.F_mean !=0) # filter()

power_summary_d40_RR2 <- power_summary_d40_RR %>% subset(power_summary_d40_RR$se.d40_mean !=0) 


# meta-meta-analysis on power
MMA_power.F_RR <- with(power.F_summary_RR2, rma.mv(yi = Mean, V = se.F_mean, random = list(~1|MA_case), method = "REML", test = "z"))
...
power.M_summary_RR2 <- power.M_summary_RR %>% subset(power.M_summary_RR$se.M_mean < 20) # remove potential outlier
MMA_power.M_RR <- with(power.M_summary_RR2, rma.mv(yi = Mean, V = se.M_mean, random = list(~1|MA_case), method = "REML", test = "z"))


MMA_power_RR_d40 <- with(power_summary_d40_RR2, rma.mv(yi = Mean, V = se.d40_mean, random = list(~1|MA_case), method = "REML", test = "z"))
...
for (i in 1:12) {
  lnVR[[i]] <- lnVR[[i]][!is.na(lnVR[[i]]$power.R_VR),]
}

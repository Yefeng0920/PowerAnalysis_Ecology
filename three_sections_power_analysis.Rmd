...
effect_size_ub <- as.numeric(model1$ci.ub) +  blups$Study[index,4] + blups$Unit_ID[,4]
effect_size_lb <- as.numeric(model1$ci.lb) + blups$Study[index,3] + blups$Unit_ID[,3]
effect_size_se <- (effect_size_ub-effect_size_lb)/(2*1.96)
...
r_power2 <- mapply(power.ma_Shinichi, mu=effect_size, sqrt(effect_size_se))
...
unlist(ES_number) %>% length() # 4601 this is to show how many global change experiments we use in our analysis 
...
for (i in 1:length(model_list_RR)) {
  blups.list[i] <- ranef(model_list_RR[[i]]) %>% list()
}

# get study index
index.list_RR <- NA # list()
for (i in 1:length(model_list_RR)) {
  index.list_RR[i] <- match(as.character(dat_list[[i]]$Study),rownames(blups.list[[i]]$Study)) %>% list()
}

# effects geting rid of random effects
effect.p_RR <- list()
for (i in 1:length(model_list_RR)) {
  effect.p_RR[i] <- mapply(sum, model_list_RR[[i]]$beta, blups.list[[i]]$Study[index.list[[i]],1], blups.list[[i]]$Unit_ID[,1]) %>% list()
}

power_RR2 <- NA
for (i in 1:length(dat_list)) {
  power_RR2[i] <- power.individual_Shinichi(mu=effect.p[[i]], se=sqrt(dat_list[[i]]$var.LRR)) %>% list()}

# plot(power_RR[[1]]-power_RR2[[1]]) - check discrepancy
# Allocate each set of power into corresponding dataset
for (i in 1:length(power_RR2)) {
...
power_S <- NA
for (i in 1:length(dat_list)) {
  power_S[i] <- mapply(type_s, A=effect.p[[i]], s=sqrt(dat_list[[i]]$var.LRR), alpha=0.05) %>% list()
}

for (i in 1:length(power_S)) {
  power_S[i] <- mapply(unlist, power_S[[i]]) %>% data.frame()
...
power_M <- NA
for (i in 1:length(dat_list)) {
  power_M[i] <- mapply(type_m, A=effect.p[[i]], s=sqrt(dat_list[[i]]$var.LRR), alpha=0.05, df=Inf, n.sims=10000) %>% list()
}  

for (i in 1:length(power_M)) {
  power_M[i] <- mapply(unlist, power_M[[i]]) %>% data.frame()
} # convert into dataframe - early format contains column names
...
# delete se=0 - some original datasets were lacking sampling errors
power_summary_RR_correct_se <- power_summary_RR %>% subset(power_summary_RR$se.mean !=0) # filter()

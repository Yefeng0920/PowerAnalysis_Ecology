...
# log
MMA_MA.S_RR2 <- lm(log(MA.power.S)~1, weights = k, data = model_est_RR)
MMA_MA.S_RR2$coefficients %>% exp() 
# logit
...
MMA_power.F.S_RR2 <- lmer(log(power.F.S_RR) ~ 1 + (1 | studyID_RR), data = individual_est_RR)
summary(MMA_power.F.S_RR2)$coefficients[1] %>% exp()
